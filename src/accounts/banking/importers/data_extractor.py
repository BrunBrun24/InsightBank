...
from accounts.shared.extract_file import extract_file_xlsx_xls
...
class DataExtractor:
    ...
    def __init__(self, bank_account_id: int, master: ctk.CTkFrame | ctk.CTk | None = None) -> None:
    ...
    def run_extraction(self) -> pd.DataFrame | None:
        ...
        bank_account_sources = {"Non précisé": "*.xls *.xlsx", "BNP Paribas": "*.xls *.xlsx *.csv"}
        ...
        for file_path in file_paths:
            ...
            if source == "BNP Paribas":
                if extension in [".xls", ".xlsx"]:
                    raw_rows = extract_file_xlsx_xls(file_path, extension)
                    df_temp = self.__extract_bnp_paribas(raw_rows)
                elif extension == ".csv":
                    df_temp = self.__extract_csv_bnp_paribas(file_path)
            else:
                if extension in [".xls", ".xlsx"]:
                    raw_rows = extract_file_xlsx_xls(file_path, extension)
                    df_temp = self.__extract_file_data(raw_rows, file_path)
        ...
        if all_dfs:
            ...
            df["bank_account_id"] = self.__bank_account_id

            return df

        return None

    def __extract_file_data(self, raw_rows: list, file_path: str) -> pd.DataFrame | None:
        try:
            ...
            operations_df[date_col] = operations_df[date_col].apply(excel_date_to_datetime)
            ...
            messagebox.showerror(
                "Erreur de donnée",
                f"Erreur sur la mise en forme du fichier {file_path}.\n\nVérifiez vos données.",
            )
            return None

    def __extract_csv_bnp_paribas(self, file_path: str) -> pd.DataFrame | None:
        try:
            df = pd.read_csv(
                file_path,
                sep=";",
            ...
            messagebox.showerror(
                "Erreur CSV",
                f"Erreur sur le fichier {file_path}.\n\nVous ne devez pas modifier le fichier que vous avez téléchargé sur votre banque.",
            )
            return None
    ...
    def __extract_bnp_paribas(self, raw_rows: list) -> pd.DataFrame:
        try:
            ...
            if missing:
                messagebox.showerror(
                    "Erreur",
                    f"Il manque des colonnes : {missing}\n\nVos nom de colonnes doivent commencer sur la première ligne.",
                )
                return None
    ...
    @staticmethod
    def __clean_libelle_spacing(libelle: str) -> str:
    ...
    @staticmethod
    def __extract_between_slashes(text: str, start: int, end: int) -> str:
