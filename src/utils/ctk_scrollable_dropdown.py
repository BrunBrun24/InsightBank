...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    def __init__(
        ...
        **button_kwargs,
        ...
        self.attach.winfo_toplevel().bind(
            "<Configure>", lambda e: self._withdraw() if not self.disable else None, add="+"
        )
        self.attach.winfo_toplevel().bind(
            "<ButtonPress>", lambda e: self._withdraw() if not self.disable else None, add="+"
        )
        ...
        self.button_color = (
            customtkinter.ThemeManager.theme["CTkFrame"]["top_fg_color"] if button_color is None else button_color
        )
        self.text_color = (
            customtkinter.ThemeManager.theme["CTkLabel"]["text_color"] if text_color is None else text_color
        )
        self.hover_color = (
            customtkinter.ThemeManager.theme["CTkButton"]["hover_color"] if hover_color is None else hover_color
        )
    ...
    def place_dropdown(self):
        ...
        if self.resize:
            ...
            self.height_new = min(self.height_new, self.height)
